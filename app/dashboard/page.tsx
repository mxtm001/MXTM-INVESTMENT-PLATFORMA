...
import { DollarSign, TrendingUp, ArrowUpRight, ArrowDownLeft, Wallet, Bell, User, LogOut, Edit } from "lucide-react"
  ...
  const handleBalanceUpdate = () => {
    ...
    try {
      ...
      if (userIndex !== -1) {
        ...
        // Update state
        setUser((prev: any) => ({ ...prev, balance: amount }))
        setShowBalanceEditor(false)
        setNewBalance("")

        toast({
          title: "Balance Updated",
          description: `Balance updated to $${amount.toLocaleString()}`,
        })
      }
    } catch (error) {
      toast({
          ...
          <Card className="bg-[#0a1735] border-[#253256] md:col-span-2">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <div className="flex items-center space-x-2">
                <Dialog open={showBalanceEditor} onOpenChange={setShowBalanceEditor}>
                  <DialogTrigger asChild>
                    <Button variant="ghost" size="sm">
                      <Edit className="h-4 w-4" />
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="bg-[#0a1735] border-[#253256]">
                    <DialogHeader>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex space-x-2">
                        <Button onClick={handleBalanceUpdate} className="bg-[#f9a826] hover:bg-[#f9a826]/90 text-black">
                          Update Balance
                        </Button>
                        <Button variant="outline" onClick={() => setShowBalanceEditor(false)}>
                          Cancel
                        </Button>
                      </div>
                    </div>
